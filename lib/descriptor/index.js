var UDF = require( '../udf' )
var debug = require( 'debug' )( 'udf:descriptor' )
...
Descriptor.PrimaryVolume = require( './primary-volume' )
...
Descriptor.LogicalVolume = require( './logical-volume' )
...
Descriptor.PartitionHeader = require( './partition-header' )

Descriptor.FileSet = require( './file-set' )
...
Descriptor.get = function( typeId ) {
  switch( typeId ) {
    // Main VDS
    case Descriptor.PVD: return Descriptor.PrimaryVolume
    ...
    case Descriptor.LVD: return Descriptor.LogicalVolume
...
Descriptor.parse = function( buffer, offset ) {
  ...
  var typeId = buffer.readUInt16LE( offset )
  var desc = Descriptor.get( typeId )

  debug( desc.name, typeId )
